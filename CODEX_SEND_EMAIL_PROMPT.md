...
3. Проверь, что после замены нет локальных путей вида `assets/...`.
4. Проверь, что хотя бы `logo-abcentrum.png` открывается по HTTPS.
...
Если `ASSET_BASE_URL` отсутствует или изображения не открываются, не отправляй рассылку и напиши точную причину.
...
Перед отправкой проверь, что ссылки не изменены.
...
2. Провалидируй и дедуплицируй адреса.
3. Проверь наличие обязательных переменных:
   - `SMTP_HOST`;
   - `SMTP_PORT`;
   - `SMTP_SECURE`;
   - `SMTP_USER`;
...
4. Подготовь HTML:
   ...
   - проверь отсутствие локальных путей `assets/...`.
...
Если пользователь сразу написал «отправляй письмо на эти адреса», можно выполнять основную рассылку без дополнительного подтверждения, но сначала проверь все обязательные настройки.
...
- Перед отправкой проверь `docs/qa-checklist.md`.
